...
struct skiplist *skiplist_new(void)
{
	...
	struct skiplist_node *header = create_node(0, 0, NULL, SKIPLIST_MAX_LEVEL);
	if (!header) {
		free(list);
		return NULL;
	}
...
void skiplist_free(struct skiplist *list)
{
	...
	struct skiplist_node *header = atomic_load(&list->header);
	struct skiplist_node *current = get_unmarked(atomic_load(&header->forward[0]));
                    ...
                    uint64_t length, void *data)
{
	...
	while (true) {
		...
		struct skiplist_node *expected = succs[0];
...
int skiplist_delete(struct skiplist *list, uint64_t offset)
{
	...
	while (true) {
		...
		/* Mark level 0 last */
		struct skiplist_node *succ = atomic_load(&victim->forward[0]);
		while (true) {
			if (is_marked(succ))
...
int skiplist_delete_node(struct skiplist *list, struct skiplist_node *target)
{
	...
	/* Mark level 0 last */
	struct skiplist_node *succ = atomic_load(&target->forward[0]);
	while (true) {
		if (is_marked(succ))
			return -1;  /* Already deleted by another thread */
...
void skiplist_print(struct skiplist *list)
{
	...
	printf("Skiplist (count=%lu, max_level=%d):\n",
	       atomic_load(&list->count), atomic_load(&list->max_level));

	while (current) {
		printf("  [%lu, %lu) level=%d data=%p marked=%d\n",
		       current->offset,
